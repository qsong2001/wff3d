...
from ..watermark.gadm_aap import sample_prune_ratio
...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        gadm_aap_config: GADMAAPConfig | None = None,
        clean_gaussian_head: nn.Module | None = None,
        robust_attacks: bool = False,
        robust_weight: float = 0.02,
        ...
        # Watermark extractor (frozen)
        self.watermark_extractor = watermark_extractor
        if self.watermark_extractor is not None:
            self.register_buffer("watermark_key", watermark_key, persistent=True)
        self.watermark_weight = float(watermark_weight)
        self.gadm_aap_config = gadm_aap_config or GADMAAPConfig()
        self.robust_attacks = bool(robust_attacks)
        self.robust_weight = float(robust_weight)
        self.attack_config = attack_config or WatermarkAttackConfig()
        self.clean_gaussian_head = clean_gaussian_head
        if self.clean_gaussian_head is not None:
            self.clean_gaussian_head.eval()
            for parameter in self.clean_gaussian_head.parameters():
    ...
    def _decode_watermark(
        ...
        if attack != "clean":
            rendered_flat = apply_image_attack(
                ...
                attack,
                self.attack_config,
                training=training_attack,
            )
        imnet_mean = rendered.new_tensor([0.485, 0.456, 0.406]).view(1, 1, 3, 1, 1)
        imnet_std = rendered.new_tensor([0.229, 0.224, 0.225]).view(1, 1, 3, 1, 1)
        ...
        # TorchScript extractors do not support checkpoint recomputation reliably.
        # BF16 model activations and checkpointed LPIPS provide the memory savings.
        decoded = self.watermark_extractor(rendered_norm)
        decoded = decoded.view(b, v, -1)
        decoded = decoded.mean(dim=1) if aggregate_views else decoded.flatten(0, 1)
        key_batch = self.watermark_key.unsqueeze(0).expand(decoded.shape[0], -1)
        loss = F.binary_cross_entropy_with_logits(decoded * 10.0, key_batch)
        bit_acc = ((decoded > 0).float() == key_batch).float().mean()
        return loss, bit_acc

    def _predict_clean_gaussians(self, context_image: Tensor, current_step: int):
        if self.clean_gaussian_head is None:
            raise RuntimeError("GADM requires a frozen clean Gaussian head")
        self.clean_gaussian_head.eval()
        trained_head = self.model.encoder.gaussian_param_head
        self.model.encoder.gaussian_param_head = self.clean_gaussian_head
        try:
            with torch.no_grad():
                clean_output = self.model.encoder(
                    context_image,
                    current_step,
                    visualization_dump=None,
                )
        finally:
            self.model.encoder.gaussian_param_head = trained_head
        return clean_output.gaussians
    ...
    @staticmethod
    def normalize_gs_for_extractor(gs):
        """Normalize gaussian attributes to match pretrained Gaussian_MAE input distribution.
        
        The pretrained PointTransformer was trained on ModelNet40 gaussians normalized via
        ModelNetGaussian.pc_norm_gs(). We replicate that normalization here.
        """
        # xyz [dim 0,1,2]: center and normalize to unit sphere
        xyz = gs[..., 0:3]
        center = xyz.mean(dim=1, keepdim=True)
        xyz = xyz - center
        radius = xyz.norm(dim=-1, keepdim=True).max(dim=1, keepdim=True)[0].clamp(min=1e-6)
        xyz_norm = xyz / radius
        # opacity [dim 3]: [0,1] -> [-1,1]
        opacity_norm = gs[..., 3:4] * 2 - 1
        # scales [dim 4,5,6]: log-transform (inverse of dataset's exp())
        scales_norm = torch.log(gs[..., 4:7].clamp(min=1e-6))
        # rotation [dim 7,8,9,10]: normalize to unit quaternion
        rot = gs[..., 7:11]
        rot_norm = rot / rot.norm(dim=-1, keepdim=True).clamp(min=1e-6)
        # sh [dim 11,12,13]: scale by 0.282, clip, scale by 2/sqrt(3)
        sh = gs[..., 11:14] * 0.28209479177387814
        sh = torch.clamp(sh, -0.5, 0.5)
        sh = sh * (2.0 / math.sqrt(3))
        return torch.cat([xyz_norm, opacity_norm, scales_norm, rot_norm, sh], dim=-1)

    def training_step(self, batch, batch_idx):
        current_step = getattr(self, "_manual_global_step", 0)
        ...
        try:
            encoder_output, output = self.model(context_image, current_step, visualization_dump=visualization_dump)
        except torch.cuda.OutOfMemoryError:
            torch.cuda.empty_cache()
            self.log("loss/total", 0.0)
        ...
        gadm_loss_value = output.color.new_zeros(())
        aap_loss_value = output.color.new_zeros(())
        aap_bit_acc = output.color.new_zeros(())
        reliability = None
        if self.gadm_aap_config.gadm or self.gadm_aap_config.aap:
            num_views = pred_context_pose["extrinsic"].shape[1]
            near = output.color.new_full((b, num_views), 0.01)
            far = output.color.new_full((b, num_views), 100.0)
            reliability = compute_gaussian_reliability(
                ...
                near,
                far,
                chunk_size=self.gadm_aap_config.reliability_chunk_size,
            )
            if self.gadm_aap_config.gadm:
                clean_gaussians = self._predict_clean_gaussians(context_image, current_step)
                gadm_loss_value = gadm_loss(
                    gaussians,
                    clean_gaussians,
                    reliability,
                    carrier=self.gadm_aap_config.gadm_carrier,
                    weighting=self.gadm_aap_config.gadm_weighting,
                    strict_carrier=self.gadm_aap_config.gadm_strict_carrier,
                )
                del clean_gaussians
        ...
        eval_ssim = None
        eval_lpips = None
        if getattr(self, "_wff3d_eval_metrics", False):
            target_flat = rearrange(target_gt, "b v c h w -> (b v) c h w")
            output_flat = rearrange(output.color.float().clamp(0, 1), "b v c h w -> (b v) c h w")
            eval_ssim = compute_ssim(target_flat, output_flat).mean()
            eval_lpips = compute_lpips(target_flat, output_flat).mean()

        if distill_infos is not None and "conf_mask" in distill_infos:
            distill_conf_mask = rearrange(distill_infos["conf_mask"], "b v h w -> (b v) h w")
        else:
            # Distillation can be disabled by config; fall back to a full valid mask.
            distill_conf_mask = rearrange(
        ...
        with torch.amp.autocast('cuda', enabled=False):
            for loss_fn in self.losses:
                loss = loss_fn.forward(output, batch, gaussians, depth_dict, current_step)
                self.log(f"loss/{loss_fn.name}", loss)
                total_loss = total_loss + loss
                loss_parts[f"loss_{loss_fn.name}"] = float(loss.detach().item())
            ...
            if depth_dict is not None and "depth" in get_cfg()["loss"].keys() and self.train_cfg.cxt_depth_weight > 0:
                ...
                loss_parts["loss_ctx_depth"] = float(loss_depth.detach().item())

            if distill_infos is not None and hasattr(self, "loss_distill") and "conf_mask" in distill_infos:
                ...
                loss_parts["loss_distill"] = float(loss_distill_list["loss_distill"].detach().item())
                loss_parts["loss_distill_pose"] = float(loss_distill_list["loss_pose"].detach().item())
                loss_parts["loss_distill_depth"] = float(loss_distill_list["loss_depth"].detach().item())
                loss_parts["loss_distill_normal"] = float(loss_distill_list["loss_normal"].detach().item())

            base_loss = total_loss

            # Watermark BCE loss on rendered images
            loss_wm = 0.0
            bit_acc = 0.0
            if self.watermark_extractor is not None:
                self.watermark_extractor.eval()
                rendered = output.color  # (b, v, c, h, w)
                _, _, _, h, w = rendered.shape

                # DEBUG: save first rendered image (step 0 only)
                if current_step == 0:
                    from torchvision.utils import save_image as tv_save
                    save_dir = Path("output/exp_dl3dv/debug_images")
                    save_dir.mkdir(parents=True, exist_ok=True)
                    img = rendered[0, 0].detach().cpu()
                    tv_save(img, save_dir / "step0_rendered.png")
                    print(f"[DEBUG] Saved {save_dir / 'step0_rendered.png'}  shape={rendered.shape} range=[{rendered.min():.3f},{rendered.max():.3f}]")

                try:
                    loss_wm, bit_acc = self._decode_watermark(rendered)
                except torch.cuda.OutOfMemoryError:
                    torch.cuda.empty_cache()
                    loss_wm = rendered.sum() * 0.0
                    bit_acc = rendered.new_zeros(())
                self.log("loss/watermark", loss_wm)
                total_loss = total_loss + self.watermark_weight * loss_wm
                self.log("metric/bit_acc", bit_acc)

                attack_name = "clean"
                if self.robust_attacks and not getattr(self, "_wff3d_eval_metrics", False):
                    attack_name = sample_training_attack(rendered.device)
                    robust_loss, robust_bit_acc = self._decode_watermark(
                        rendered,
                        attack=attack_name,
                        training_attack=True,
                    )
                    total_loss = total_loss + self.robust_weight * robust_loss
                    self.log("loss/robust_watermark", robust_loss)
                    self.log("metric/robust_bit_acc", robust_bit_acc)
                    loss_parts["loss_robust"] = float(robust_loss.detach().item())
                    loss_parts["robust_bit_acc"] = float(robust_bit_acc.detach().item())

                if getattr(self, "_wff3d_eval_attacks", False):
                    self.latest_attack_metrics = {}
                    for eval_attack in ATTACK_NAMES:
                        _, attacked_acc = self._decode_watermark(
                            rendered,
                            attack=eval_attack,
                        )
                        self.latest_attack_metrics[eval_attack] = float(attacked_acc.detach().item())

                if self.gadm_aap_config.gadm:
                    total_loss = total_loss + self.gadm_aap_config.lambda_gadm * gadm_loss_value
                    self.log("loss/gadm", gadm_loss_value)
                    loss_parts["loss_gadm"] = float(gadm_loss_value.detach().item())

                if self.gadm_aap_config.aap:
                    prune_ratio = sample_prune_ratio(self.gadm_aap_config, rendered.device)
                    pruned_gaussians, keep_mask = prune_gaussians(
                        gaussians,
                        reliability,
                        prune_ratio,
                        strategy=self.gadm_aap_config.aap_strategy,
                    )
                    num_aap_views = min(
                        self.gadm_aap_config.aap_num_views,
                        rendered.shape[1],
                    )
                    aap_view_indices = torch.randperm(
                        rendered.shape[1], device=rendered.device
                    )[:num_aap_views]
                    pruned_output = self.model.decoder.forward(
                        pruned_gaussians,
                        pred_context_pose["extrinsic"][:, aap_view_indices],
                        pred_context_pose["intrinsic"][:, aap_view_indices],
                        rendered.new_full((b, num_aap_views), 0.01),
                        rendered.new_full((b, num_aap_views), 100.0),
                        (h, w),
                        "depth",
                    )
                    aap_loss_value, aap_bit_acc = self._decode_watermark(pruned_output.color)
                    total_loss = total_loss + self.gadm_aap_config.lambda_aap * aap_loss_value
                    self.log("loss/aap", aap_loss_value)
                    self.log("metric/aap_bit_acc", aap_bit_acc)
                    loss_parts["loss_aap"] = float(aap_loss_value.detach().item())
                    loss_parts["aap_bit_acc"] = float(aap_bit_acc.detach().item())
                    loss_parts["aap_prune_ratio"] = float(prune_ratio.detach().item())
                    loss_parts["aap_keep_ratio"] = float(keep_mask.float().mean().item())
                    loss_parts["aap_num_views"] = float(num_aap_views)
        ...
        LOSS_THRESHOLD = 999.0  # disabled for watermark training
        if current_step > SKIP_AFTER_STEP and total_loss > LOSS_THRESHOLD:
            print(f"Skipping batch with high loss ({total_loss:.6f}) at step {current_step} on Rank {self.global_rank}")
        ...
        self.latest_train_metrics = {
            ...
            "anysplat_loss": float(base_loss.detach().item() if torch.is_tensor(base_loss) else base_loss),
            "watermark_loss": float(loss_wm.detach().item() if torch.is_tensor(loss_wm) else loss_wm),
            "bit_acc": float(bit_acc.detach().item() if torch.is_tensor(bit_acc) else bit_acc),
            "psnr": float(psnr_probabilistic.mean().detach().item()),
            "ssim": float(eval_ssim.detach().item()) if eval_ssim is not None else None,
            "lpips": float(eval_lpips.detach().item()) if eval_lpips is not None else None,
            "parts": loss_parts,
        }
            
        self.log("info/global_step", current_step)  # hack for ckpt monitor
        
        # Tell the data loader processes about the current step.
        if self.step_tracker is not None:
            self.step_tracker.set_step(current_step)
    ...
    @staticmethod
    def _trainable_gaussian_decoder_prefixes() -> tuple[str, ...]:
        return (
            "model.decoder.",
            "model.encoder.gaussian_param_head",
            "model.encoder.interm",
        )

    def on_save_checkpoint(self, checkpoint: dict) -> None:
        # Keep Lightning trainer states (optimizer/scheduler/loops/global_step) intact,
        # but persist only trainable gaussian/decoder model weights.
        state_dict = checkpoint.get("state_dict", {})
        prefixes = self._trainable_gaussian_decoder_prefixes()
        checkpoint["state_dict"] = {
            k: v for k, v in state_dict.items() if any(k.startswith(p) for p in prefixes)
        }

    def on_load_checkpoint(self, checkpoint: dict) -> None:
        # Expand filtered checkpoint weights back to a full model state dict before strict load.
        filtered_state_dict = checkpoint.get("state_dict", {})
