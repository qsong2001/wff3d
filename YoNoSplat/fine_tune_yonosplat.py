...
def _load_full_checkpoint(model_wrapper: ModelWrapper, ckpt_path: str) -> None:
    print(cyan(f"[YoNoSplat] loading checkpoint: {ckpt_path}"))
    ckpt = torch.load(ckpt_path, map_location="cpu")
    state = ckpt["state_dict"] if isinstance(ckpt, dict) and "state_dict" in ckpt else ckpt
    missing, unexpected = model_wrapper.load_state_dict(state, strict=True)
    print(cyan(f"[YoNoSplat] strict checkpoint load ok. missing={len(missing)} unexpected={len(unexpected)}"))
...
def _load_tuned_checkpoint(model_wrapper: ModelWrapper, ckpt_path: str) -> int:
    ...
    print(cyan(f"[YoNoSplat] tuned checkpoint loaded strictly: {ckpt_path} step={step}"))
    return step
...
def _set_trainable_modules(model_wrapper: ModelWrapper) -> list[torch.nn.Parameter]:
    ...
    # Keep the official encoder training mode so pose-free scheduled sampling
    # can choose GT or predicted context poses. Parameters remain frozen below.
    model_wrapper.train()
    model_wrapper.encoder.train()
    model_wrapper.decoder.eval()

    trainable_modules = [
        model_wrapper.encoder.gaussian_decoder,
        model_wrapper.encoder.gaussian_head,
        model_wrapper.encoder.rgb_embed,
    ]
    ...
    print(cyan(f"[YoNoSplat] trainable params: {total / 1e6:.2f}M"))
    return params
...
def evaluate_smoke(
    ...
    eval_shard_index: int = 0,
    eval_num_shards: int = 1,
    ...
    if not 0 <= eval_shard_index < eval_num_shards:
        raise ValueError(
            f"eval-shard-index must be in [0, {eval_num_shards}), got {eval_shard_index}"
        )
    for source_batch_index, batch in enumerate(data_loader):
        if source_batch_index % eval_num_shards != eval_shard_index:
            continue
        if device.type == "cuda":
        ...
        if cfg.model.encoder.pose_free:
            output = model_wrapper.test_step_align(
                batch,
                gaussians,
                visualization_dump["c2w"],
            )
        else:
        ...
        for attack_name in GAUSSIAN_ATTACK_NAMES if evaluate_gaussian_attacks else ():
            attacked_gaussians = apply_gaussian_attack(
                gaussians, attack_name, gaussian_attack_config
            )
            if cfg.model.encoder.pose_free:
                # Camera alignment is part of the clean inference protocol, not the attack.
                # Reuse it so every attacked render is measured from the exact same view.
                attacked_output = model_wrapper.decoder.forward(
                    attacked_gaussians,
                    output.aligned_extrinsics,
                    batch["target"]["intrinsics"],
                    batch["target"]["near"],
                    batch["target"]["far"],
                    target.shape[-2:],
        ...
        print(f"[WFF3D-EVAL][YoNoSplat] batch={count} bit_acc={float(bit_acc):.4f}")
        if count >= num_batches:
            break
    if count == 0:
        raise RuntimeError("YoNoSplat smoke evaluation received no batches")
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if global_step % print_every == 0:
                ...
                print(
                    ...
                    f"yono_loss={base_loss.detach().item():.4f}; "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="WwF3D manual YoNoSplat fine-tuner")
    parser.add_argument("--config-path", type=str, default="config/main.yaml")
    parser.add_argument("--experiment", type=str, default="config/experiment/yono_dl3dv.yaml")
    parser.add_argument("--ckpt", type=str, required=True, help="Full YoNoSplat Lightning checkpoint")
    parser.add_argument("--dataset-root", type=str, default=None, help="Raw DL3DV root for dl3dv_raw")
    ...
    overrides = []
    if args.experiment:
        overrides.append(f"+experiment={Path(args.experiment).stem}")
    if args.dataset_root:
        overrides.extend(
            [
                "dataset.dl3dv.name=dl3dv_raw",
                f"dataset.dl3dv.roots=[{args.dataset_root}]",
                "dataset.dl3dv.original_image_shape=[540,960]",
                "dataset.dl3dv.skip_bad_shape=false",
            ]
        )
    if args.eval_index:
        overrides.extend(
            [
                ...
                f"dataset.dl3dv.view_sampler.index_path={args.eval_index}",
                f"dataset.dl3dv.view_sampler.num_context_views={args.eval_context_views}",
    ...
    output_dir = (
        ...
        if args.output_dir
        else Path(f"outputs/exp_wff3d_yonosplat/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}")
    )
    ...
    model_wrapper = ModelWrapper(
        ...
        encoder,
        encoder_visualizer,
        get_decoder(cfg_typed.model.decoder),
        get_losses(cfg_typed.loss),
        step_tracker,
        gaussian_downsample_ratio=cfg_typed.model.encoder.gaussian_downsample_ratio,
        gaussians_per_axis=cfg_typed.model.encoder.gaussians_per_axis,
    )
    ...
    optimizer = torch.optim.AdamW(params, lr=cfg_typed.optimizer.lr, weight_decay=0.01)
    ...
    if args.eval_only:
        ...
        evaluate_smoke(
            ...
            args.eval_shard_index,
            args.eval_num_shards,
